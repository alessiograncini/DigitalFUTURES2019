...
public class FlatLightingShaderEditor : ShaderGUI {
	...
	private MaterialProperty lightAndTextureMix = null;
	...
	private ValueWrapper<bool> useMainTexture = false;
	...
	protected virtual void FindProperties (MaterialProperty[] props) {
		...
		lightAndTextureMix = FindProperty("_LightAndTextureMix", props);
		...
		if (shouldShowLightingProperties) {
			using (new UITools.GUIVertical(UITools.VGroupStyle)) {
				...
				using (new UITools.GUIDisable(IsShaderBaked(material))) {
					UITools.ToggleHeader(useThreeLightSystem, Labels.UseSymmetrcialColors);
				}
				...
				ShowMainTextureOption();
			}
		}
	}

	private void ShowMainTextureOption()
	{
		using(new UITools.GUIDisable(IsShaderBaked(material)))
		{
			UITools.ToggleHeader(useMainTexture, Labels.UseMainTexture);
		}
		if(useMainTexture)
		{
			material.EnableKeyword(Constants.Shader.USE_MAIN_TEXTURE_KEYWORD);
			materialEditor.TextureProperty(mainTexture, Labels.MainTextureLabel, true);
			materialEditor.ShaderProperty(lightAndTextureMix, Labels.TexturePower);
		}
		else
		{
	...
	private void ShowVertexColorOption() {
		using (new UITools.GUIDisable(IsShaderBaked(material))) {
			UITools.ToggleHeader(useVertexColors, Labels.VertexColor);
		}
