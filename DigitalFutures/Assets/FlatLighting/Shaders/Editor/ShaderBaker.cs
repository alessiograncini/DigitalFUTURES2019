...
namespace FlatLighting {
	...
	public class ShaderBaker : EditorWindow {
		...
		private static bool overrite;
		private static bool disableBatching;
		public static Material targetMaterial;

		private static readonly string BAKED_SHADERS_RELATIVE_PATH = 
			Path.DirectorySeparatorChar + "Shaders"+
			Path.DirectorySeparatorChar + "Baked" + 
		...
		public static void ShowWindow(Material _material) {
			...
			ShaderBaker.overrite = false;
			ShaderBaker.disableBatching = false;
		...
		private static void OptimizeMaterial(Material _material, bool disableBatching) {
			...
			for (int i = 0; i < shaderStringRows.Count; i++) {
				...
				if (disableBatching && shaderStringRows[i].Contains("\"FlatLightingDisableBatching\"=\"False\"")) {
					shaderStringRows[i] = "\"DisableBatching\"=\"True\"";
				}
		...
		public static string FindBakedShadersRelativePath() {
			...
			if (results.Length > 0) {
				...
				p = p.Substring(0, p.LastIndexOf(Path.DirectorySeparatorChar));
				p = p.Substring(0, p.LastIndexOf(Path.DirectorySeparatorChar));
