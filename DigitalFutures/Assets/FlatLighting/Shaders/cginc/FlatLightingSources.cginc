...
#define FL_SPOT_LIGHT_LIMIT 25 //Maximum number of lights sources per light type.
	...
	inline half4 spot_light(float4 vertexWorldPosition, half3 vertexWorldNormal) {
		half4 spotLightColor;
		for (int i = 0; i < _SpotLight_Length; i++) {
			half3 vertexNormalInSpotLightSpace = (mul(_SpotLightMatrixC0[i], half4(vertexWorldNormal, 0))).xyz;
			fixed isInSight = dot(_SpotLightObjectSpaceForward[i], vertexNormalInSpotLightSpace) < 0;

			#if defined(FL_BLEND_LIGHT_SOURCES)
				fixed4 newSpotLightColor = isInSight * spot_light_color(i, vertexWorldPosition, _BlendedLightSmoothness, _BlendedLightIntensities, _BlendedLightColor);
				spotLightColor = blend_same_light_sources_types(spotLightColor, newSpotLightColor);
			#else
				spotLightColor += isInSight * spot_light_color(i, vertexWorldPosition, _SpotLightSmoothness[i], _SpotLightIntensities[i], _SpotLightColor[i]);
			#endif
		}
	...
	inline fixed4 point_light_color(int i, float4 vertexWorldPosition, fixed smoothness, half4 lightIntensities, fixed4 lightColor) {
			...
			half pointLightInfluence = sqrt(dot(pointLightC0, pointLightC0));
			half4 pointLightInflunceVector = half4(pointLightInfluence, pointLightInfluence, pointLightInfluence, pointLightInfluence);
	...
	inline half4 point_light(float4 vertexWorldPosition, half3 vertexWorldNormal) {
		half4 pointLightColor;
		for (int i = 0; i < _PointLight_Length; i++) {
			half3 vertexNormalInPointLightSpace = (mul(_PointLightMatrixC0[i], vertexWorldPosition));
			fixed isInSight = smoothstep(1, 0, dot(vertexNormalInPointLightSpace, vertexWorldNormal));

			#if defined(FL_BLEND_LIGHT_SOURCES)
				fixed4 newPointLightColor = isInSight * point_light_color(i, vertexWorldPosition, _BlendedLightSmoothness, _BlendedLightIntensities, _BlendedLightColor);
				pointLightColor = blend_same_light_sources_types(pointLightColor, newPointLightColor);
			#else
				pointLightColor += isInSight * point_light_color(i, vertexWorldPosition, _PointLightSmoothness[i], _PointLightIntensities[i], _PointLightColor[i]);
			#endif
		}
