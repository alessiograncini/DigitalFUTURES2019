...
namespace FlatLighting {
	public class CreatePlane : ScriptableWizard
	{
		...
		void OnWizardCreate()
		{
			string relativeDirectoryToAssets = "Resources";
			string absoluteAssetPath = Application.dataPath + "/" + relativeDirectoryToAssets + "/";
			string relativeAssetPath = "Assets/" + relativeDirectoryToAssets + "/";
			...
			Mesh m = (Mesh)AssetDatabase.LoadAssetAtPath(relativeAssetPath + planeAssetName,typeof(Mesh));
			...
			if (m == null)
			{
				...
				{
					Directory.CreateDirectory(absoluteAssetPath);
				}

				AssetDatabase.CreateAsset(m, relativeAssetPath + planeAssetName);
